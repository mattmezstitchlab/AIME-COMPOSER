...
## Le seuil (page d'accueil)

`index.html` is a threshold, not a front door that *does* things (wave 3 of
`AUDIT/POINT-ZERO-CONVERGENCE-01.md`): it states the rule — **one interface** —
says where each function lives inside Point Zero, and sends you there. The
universal composer it used to carry (pure resolver, preview, ＋ menu, sample
intents) now lives in the ＋ of Point Zero, which imports the same
`home-resolver.mjs` (109 tests — the pure function outlived the page).

`home.js` keeps three duties and nothing else: declarative icons, the shared
theme preference, and an honest NOEMA badge (`/api/state`). Served statically it
stays readable and says so — nothing simulates. Smokes: `design-system/qa/smoke-seuil.mjs`.
...
npm run verify  # execute the scripts of all 14 audited screens in a DOM (needs the jsdom devDependency)
...
12 automatic Design QA families · 14 audited screens (the **single vertical
documentation page** — 20 chapters, art direction included —, 11 experience
screens, the repository threshold, and the Point Zero shell — whose Bureau
absorbed the former Atlas media-library page, whose NOEMA rail absorbed the former
loop screen, and whose ＋ absorbed the former homepage composer).

The documentation is **one page**: `design-system/index.html`, generated by
`node src/merge-doc.mjs` from `src/doc-intro.html` (chapter 01) and — once, on
18 Sept. 2026 — the 19 former chapter pages, now deleted. One `<h1>`, chapter
heroes as `<h2>`, chapter-prefixed anchors (`#color-contraste`), the sidebar of
`js/doc.js` follows the chapter being read. The experience screens stay separate
pages: they are screens, not chapters.

Chapter 21, `design-system/index.html#direction`, picks an artistic direction **inside** the system
...
It is the same engine that audits the 14 screens of the system, not a copy, so a
...
`design-system/qa/smoke-point-zero-bureau.mjs` (which also proves the universal
resolver in the ＋ since wave 3); the threshold has its own `smoke-seuil.mjs`:

```bash
cd design-system && node qa/smoke-point-zero-bureau.mjs && node qa/smoke-point-zero-loop.mjs && node qa/smoke-seuil.mjs
```
...
`design-system/qa/smoke-point-zero-bureau.mjs` (52 checks, jsdom), the gate of
...
It is one of the 14 audited screens. Its patterns (`pz-*`, `dock`, `uimport`) live in
